...
def index():
    if request.method == 'GET':
        if session.get("user_id") is None:
            ...
            return render_template("search.html", user_id=user_id, login=session["login"])
        ...
        if db.execute("SELECT username from users WHERE username=:username", {"username": username}).rowcount == 0:
            ...
            if bcrypt.check_password_hash(db_password, password):
                ...
                return render_template("search.html", user_id=session["user_id"], login=session["login"])
            else:
                message = "Invalid username or password"
...
def search():
    if request.method == 'GET':
        ...
        if db.execute(
            ...
            return render_template("search.html", danger_message=message)
        else:
...
def book(book_id):
    ...
    if request.method == 'GET':
        ...
        if rating < 0 or len(review) <= 0:
            ...
            return render_template("book.html", book=book, danger_message=message)
        else:
            ...
            return render_template("book.html", book=book, success_message=message)
